...
<div class="unit-card" style="background:{card_bg}; border-top:3px solid {d_color}; border:1px solid {card_border}; border-top:3px solid {d_color}; border-radius:6px; padding:10px; display:flex; flex-direction:column; box-sizing:border-box;">
  ...
  <div style="margin-bottom:6px; min-height:72px; display:flex; flex-direction:column; justify-content:flex-start;">
  ...
  <div style="display:grid; grid-template-columns:1fr 1fr; gap:4px;">
    ...
    return (
        '<style>'
        '.unit-card-grid { display:grid; gap:10px; align-items:stretch; width:100%; box-sizing:border-box; grid-auto-rows:1fr; }'
...
if st.session_state['csvs_ready']:
    ...
    _area_signature = (tuple(selected_names), round(float(area_sq_mi), 2), float(resp_radius_mi), float(guard_radius_mi), int(n))
    if st.session_state.get('auto_fleet_signature') != _area_signature and not df_curve.empty:
        try:
            _resp_series = df_curve['Responder (Calls)'].dropna()
            _resp_target = _resp_series[_resp_series >= 85.0].first_valid_index()
            if _resp_target is None:
                _resp_target = _resp_series.last_valid_index()
            _resp_default = max(2, int(df_curve.loc[_resp_target, 'Drones']) if _resp_target is not None else 2)
            _resp_default = min(_resp_default, max(1, max_resp_calc))
            _guard_default = 1 if max_guard_calc >= 1 else 0
            st.session_state['k_resp'] = _resp_default
            st.session_state['k_guard'] = _guard_default
            st.session_state['auto_fleet_signature'] = _area_signature
            st.rerun()
        except Exception:
            st.session_state['auto_fleet_signature'] = _area_signature
    ...
    main_logo_html = f'<img src="data:image/png;base64,{logo_b64}" style="height:42px; vertical-align:middle; margin-right:15px;">' if logo_b64 else f'<span style="font-size:1.5rem; font-weight:900; letter-spacing:2px; color:#ffffff; margin-right:15px;">BRINC</span>'
    ...
    if fleet_capex > 0:
        ...
        logo_html_str = f'<img src="data:image/png;base64,{logo_b64_dark}" style="height:40px;">' if logo_b64_dark else '<div style="font-size:24px;font-weight:900;letter-spacing:3px;color:#fff;">BRINC</div>'
...
/* ── FOOTER ──────────────────────────────────────────────────── */
.doc-footer{{background:var(--ink);color:#cbd5e1;padding:28px 42px;font-size:11px;display:flex;justify-content:space-between;align-items:center;gap:20px;flex-wrap:wrap}}
.doc-footer a{{color:#e5e7eb;text-decoration:none}}
...
@media print{{
  ...
  .cover-page,.doc-section,.doc-footer,.disc{{page-break-inside:avoid; break-inside:avoid;}}
  .doc-section,.cover-page{{page-break-after:always; break-after:page; border-bottom:none; min-height:auto; padding:34px 42px;}}
  .doc-section:last-of-type,.doc-footer{{page-break-after:auto; break-after:auto;}}
  table, tr, td, th {{page-break-inside:avoid; break-inside:avoid;}}
  .map-wrap, .grant-layout, .infra-grid {{page-break-inside:avoid; break-inside:avoid;}}
}}
</style>
</head>
<body>
...
<nav class="doc-sidebar">
  <div class="sidebar-logo">
    {"<img src='data:image/png;base64," + logo_b64_light + "' alt='BRINC'>" if logo_b64_light else "<div class='brand'>BRINC</div>"}
  </div>
...
<section class="cover-page" id="cover">
  <div class="cover-logo">
    {"<img src='data:image/png;base64," + logo_b64_dark + "' alt='BRINC'>" if logo_b64_dark else "<div class='brand'>BRINC</div>"}
  </div>
  <div class="cover-body">
    ...
    <div class="cover-right">
      <img class="product-img" src="data:image/png;base64,{product_b64}" alt="BRINC GIG">
    </div>
  </div>
...
<footer class="doc-footer">
  <span class="brand-mark">{"<img src='data:image/png;base64," + logo_b64_dark + "' style='height:18px;vertical-align:middle;' alt='BRINC'>" if logo_b64_dark else "BRINC"}</span>
  <span>{"<img src='data:image/png;base64," + logo_b64_dark + "' style='height:24px;vertical-align:middle;'>" if logo_b64_dark else ""} BRINC Drones, Inc. · <a href="https://brincdrones.com">brincdrones.com</a> · <a href="mailto:sales@brincdrones.com">sales@brincdrones.com</a> · +1 (855) 950-0226</span>

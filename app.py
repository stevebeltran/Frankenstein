...
if st.session_state['csvs_ready']:
    ...
    with st.spinner(get_faa_message()):
        faa_geojson = faa_rf.faa_rf.load_faa_parquet(minx, miny, maxx, maxy)
    ...
    with st.spinner(get_airfield_message()):
        airfields = faa_rf.faa_rf.fetch_airfields(minx, miny, maxx, maxy)
    ...
    for idx, d_type in ordered_deployments_raw:
        ...
        d = {
            ...
            'faa_ceiling': faa_rf.faa_rf.get_station_faa_ceiling(d_lat, d_lon, faa_geojson),
            'nearest_airport': faa_rf.faa_rf.get_nearest_airfield(d_lat, d_lon, airfields)
        }
    ...
    with map_col:
        ...
        if show_faa and faa_geojson and faa_geojson.get("features"):
            try:
                faa_rf.faa_rf.add_faa_laanc_layer_to_plotly(fig, faa_geojson, is_dark=not show_satellite)
            except Exception as e:
                st.sidebar.error(f"🔴 FAA render error: {str(e)[:100]}")

        if show_obstacles:
            faa_rf.faa_rf.add_faa_obstacles_layer_to_plotly(fig, minx, miny, maxx, maxy)
    ...
    if _rf_mode == "Advanced (Grid-Based Coverage Probability)":
        ...
        if active_drones:
            ...
            for _adv_idx, _adv_stn in enumerate(active_drones):
                ...
                _adv_grid = faa_rf.faa_rf._compute_rf_grid_coverage(
                    tx_lat=_adv_stn['lat'],
                    tx_lon=_adv_stn['lon'],
            ...
            for _stn_name, _stn_grid in _adv_all_grids:
                ...
                _adv_fig = faa_rf.faa_rf._plot_rf_coverage_heatmap(
                    _stn_grid,
                    _stn_name,
        ...
        _rf_rings = faa_rf.faa_rf._rf_range_rings_3390(
